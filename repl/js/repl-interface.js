...
class MarkdownUIREPL {
  ...
  setupEventListeners() {
    ...
    // WASM reload button (if it exists)
    const reloadBtn = document.getElementById('wasm-reload-btn');
    if (reloadBtn) {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      // Add Ctrl+Shift+R for manual WASM reload
  ...
  async reloadWasm() {
    if (!this.rubyWasm) return;

    try {
      this.updateStatus('Reloading WASM...', 'status-loading');
      this.updateWasmStatus('Reloading...', 'status-loading');

      await this.rubyWasm.forceReload();

      this.updateStatus('WASM reloaded successfully', 'status-ready');
      this.updateWasmStatus('Loaded', 'status-ready');

      // Re-run the current code after reload
      if (this.editor && this.editor.getValue().trim()) {
        setTimeout(() => this.runCode(), 500);
      }

    } catch (error) {
      console.error('Manual WASM reload failed:', error);
      this.updateStatus('WASM reload failed: ' + error.message, 'status-error');
      this.updateWasmStatus('Reload failed', 'status-error');
    }
  }
  ...
  insertElement(elementName) {
    this.elementExamples = {
      ...
      // Module Components
      checkbox: '__Checkbox|\n  __Input|checkbox|type="checkbox"__\n  __Label|I agree to the terms__\n__',
      dimmer: '__Dimmer|\n  __Content|\n    __Header|Dimmed Content__\n    This content is dimmed\n  __\n__',
      embed: '__Embed|https://www.youtube.com/embed/dQw4w9WgXcQ__',
      modal: '__Modal|sample-modal__\n  __Header|Modal Title__\n  __Content|\n    __Description|\n      This is modal content\n    __\n  __\n  __Actions|\n    __Button|Cancel|secondary__\n    __Button|OK|primary__\n  __\n__',
      popup: '__Popup|trigger__\n  __Content|\n    This is popup content\n  __\n__',
      rating: '__Rating|5|3__',
      search: '__Search|\n  __Input|placeholder="Search..."__\n  __Results__\n__',
      shape: '__Shape|\n  __Side|\n    Front Side Content\n  __\n  __Side|\n    Back Side Content\n  __\n__',
      sidebar: '__Sidebar|\n  __Item|Menu Item 1__\n  __Item|Menu Item 2__\n__',
      sticky: '__Sticky|\n  This content sticks to viewport\n__',
      transition: '__Transition|fade__\n  Content with transitions\n__',

      // Element Components
      placeholder: '__Placeholder|\n  __Header|\n    __Line__\n    __Line__\n  __\n  __Paragraph|\n    __Line__\n    __Line__\n    __Line__\n  __\n__',
      rail: '__Rail|left__\n  Left rail content\n__',
      reveal: '__Reveal|\n  __Visible Content|\n    Hover me\n  __\n  __Hidden Content|\n    Hidden content\n  __\n__',
      step: '__Steps|\n  __Step|\n    __Icon|truck__\n    __Content|\n      __Title|Shipping__\n      __Description|Choose method__\n    __\n  __\n  __Step|active__\n    __Icon|payment__\n    __Content|\n      __Title|Billing__\n      __Description|Enter info__\n    __\n  __\n__',

      // View Components
      advertisement: '__Advertisement|leaderboard__\n  Advertisement Content\n__',
      comment: '__Comments|\n  __Comment|\n    __Avatar|\n      __Image|https://via.placeholder.com/50x50__\n    __\n    __Content|\n      __Author|John Doe__\n      __Metadata|2 hours ago__\n      __Text|This is a great project!__\n    __\n  __\n__',
      feed: '__Feed|\n  __Event|\n    __Label|\n      __Icon|pencil__\n    __\n    __Content|\n      __Summary|You posted a new message__\n      __Date|2 hours ago__\n    __\n  __\n__',
      item: '__Items|\n  __Item|\n    __Image|https://via.placeholder.com/100x100__\n    __Content|\n      __Header|Project Alpha__\n      __Meta|Started 3 days ago__\n      __Description|Description here__\n    __\n  __\n__',
      statistic: '__Statistics|\n  __Statistic|\n    __Value|1,200__\n    __Label|Downloads__\n  __\n  __Statistic|\n    __Value|500__\n    __Label|Views__\n  __\n__'
    };
    ...
    this.blockquoteExamples = {
      ...
      // Module Components (Blockquote syntax)
      checkbox: '> Checkbox:\n> __Input|checkbox|type="checkbox"__\n> __Label|I agree to the terms__',
      dimmer: '> Dimmer:\n> __Content|\n> __Header|Dimmed Content__\n> This content is dimmed',
      embed: '> Embed: https://www.youtube.com/embed/dQw4w9WgXcQ',
      popup: '> Popup: trigger\n> __Content|\n> This is popup content',
      rating: '> Rating: 5|3',
      search: '> Search:\n> __Input|placeholder="Search..."__\n> __Results__',
      shape: '> Shape:\n> __Side|\n> Front Side Content\n> __Side|\n> Back Side Content',
      ...
      // Element Components (Blockquote syntax)
      placeholder: '> Placeholder:\n> __Header|\n> __Line__\n> __Line__\n> __Paragraph|\n> __Line__\n> __Line__\n> __Line__',
      rail: '> Rail: left\n> Left rail content',
      reveal: '> Reveal:\n> __Visible Content|\n> Hover me\n> __Hidden Content|\n> Hidden content',
      step: '> Steps:\n> __Step|\n> __Icon|truck__\n> __Content|\n> __Title|Shipping__\n> __Description|Choose method__\n> __Step|active__\n> __Icon|payment__\n> __Content|\n> __Title|Billing__\n> __Description|Enter info__',

      // View Components (Blockquote syntax)
      advertisement: '> Advertisement: leaderboard\n> Advertisement Content'
    };
  ...
  async runCode() {
    ...
    try {
      ...
      // Show reload status if WASM is reloading
      if (this.rubyWasm.reloadInProgress) {
        this.updateStatus('Reloading WASM...', 'status-loading');
        this.updateWasmStatus('Reloading...', 'status-loading');
      }
      ...
      if (error.message.includes('Circuit breaker open')) {
        this.updateStatus('Too many errors - cooling down...', 'status-error');
        this.updateWasmStatus('Circuit breaker open', 'status-error');
        this.showCircuitBreakerError(error);
      }
      // Check if this is a WASM reload failure
      else if (error.message.includes('WASM reload failed') || error.message.includes('Maximum retry attempts')) {
        this.updateStatus('WASM reload failed - please refresh page', 'status-error');
        this.updateWasmStatus('Reload failed', 'status-error');
        this.showWasmReloadError();
      } else {
  ...
  showCircuitBreakerError(error) {
    const previewContainer = document.getElementById('preview-container');
    if (previewContainer) {
      const waitTime = error.message.match(/Wait (\d+)s/);
      const countdown = waitTime ? parseInt(waitTime[1]) : 5;

      previewContainer.innerHTML = `
        <div class="ui warning message">
          <div class="header">Circuit Breaker Active</div>
          <p>Too many consecutive errors detected. The system is temporarily paused to prevent further issues.</p>
          <div class="ui list">
            <div class="item">⏱️ Waiting ${countdown} seconds before allowing new attempts</div>
            <div class="item">🔄 You can manually reload WASM using Ctrl+Shift+R</div>
            <div class="item">💡 Or wait for the automatic cooldown to complete</div>
          </div>
          <div class="ui buttons">
            <div class="ui button" onclick="window.MarkdownUIREPL.reloadWasm()">
              <i class="refresh icon"></i>
              Force WASM Reload
            </div>
            <div class="ui button" onclick="location.reload()">
              <i class="redo icon"></i>
              Refresh Page
            </div>
          </div>
  ...
  showWasmReloadError() {
    ...
    if (previewContainer) {
      previewContainer.innerHTML = `
        <div class="ui error message">
          <div class="header">WASM Recovery Failed</div>
          <p>The Ruby WebAssembly module encountered critical errors and automatic recovery failed.</p>
          <div class="ui list">
            <div class="item">✗ Automatic reload attempts exhausted</div>
            <div class="item">🔄 Please refresh the page to restart</div>
            <div class="item">💡 If problems persist, clear browser cache</div>
          </div>
          <div class="ui button" onclick="location.reload()">
            <i class="refresh icon"></i>
            Refresh Page
  ...
  updatePreview(html) {
    ...
    // Sanitize HTML to prevent malformed img tags from causing URL requests
    let sanitizedHtml = html || '';

    try {
      // More comprehensive image tag sanitization
      // 1. Remove incomplete img tags that don't have proper src quotes
      sanitizedHtml = sanitizedHtml.replace(/<img\s+[^>]*src=(?!['"]).*/gi, '<span class="ui label red">Malformed image tag</span>');

      // 2. Remove img tags with empty or invalid src attributes
      sanitizedHtml = sanitizedHtml.replace(/<img\s+[^>]*src=(['"])\s*\1[^>]*>/gi, '<span class="ui label orange">Empty image source</span>');

      // 3. Sanitize any remaining malformed img tags
      sanitizedHtml = sanitizedHtml.replace(/<img(?![^>]*src=['"][^'"]+['"])[^>]*>/gi, '<span class="ui label yellow">Invalid image tag</span>');

      // 4. Validate and sanitize valid-looking img tags
      sanitizedHtml = sanitizedHtml.replace(/<img\s+([^>]*src=['"]([^'"]+)['"][^>]*)>/gi, (match, attributes, srcUrl) => {
        try {
          // Check if URL looks valid (basic validation)
          if (!srcUrl || srcUrl.trim() === '' || srcUrl.includes('<') || srcUrl.includes('>')) {
            return '<span class="ui label red">Invalid image URL</span>';
          }
          // Return the original match if it passes basic validation
          return match;
        } catch (e) {
          console.warn('Error validating image URL:', e);
          return '<span class="ui label red">Image validation error</span>';
        }
      });

    } catch (error) {
      console.error('Error sanitizing HTML:', error);
      // Fallback: remove all img tags if sanitization fails
      sanitizedHtml = sanitizedHtml.replace(/<img[^>]*>/gi, '<span class="ui label red">Image removed due to error</span>');
    }
