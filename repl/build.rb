...
# Copy WASM files to parent directory as requested
puts "📋 Copying WASM files to parent directory..."
Dir.glob(File.join(BUILD_DIR, '*.wasm')).each do |wasm_file|
  parent_dest = File.join(__dir__, File.basename(wasm_file))
  FileUtils.cp(wasm_file, parent_dest)
  puts "✓ Copied #{File.basename(wasm_file)} to #{parent_dest}"
end
